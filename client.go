...
const (
	defaultBaseURL = "https://rest.esms.vn/MainService.svc/json"
)
...
// NewClient returns a eSMS client
func NewClient(apiKey string, secretKey string) (*Client, error) {
	...
	c := &Client{
		httpClient: newHTTPClient(apiKey, secretKey),
	}
...
func (c *httpClient) NewRequest(method, path string, body interface{}) (*http.Request, error) {
	...
	c.printDebug(fmt.Sprintf("[eSMS] requesting url: %s", u.String()))
	...
	if body != nil {
		...
		if c.logger != nil {
			c.logger("[eSMS] request body: " + b.String())
		}
	}
